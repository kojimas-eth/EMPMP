...
C.post_dct = False
## Motion Network mlp
dim_ = 45#JK维度，将JK映射到D
C.motion_mlp = edict()
C.motion_mlp.hidden_dim = dim_
C.motion_mlp.seq_len = C.motion.h36m_input_length_dct#输入的帧数，用于设置T维度的线性层的维度
C.motion_mlp.num_layers = 48#层数
...
C.num_workers = 8
C.device="cuda"
C.cos_lr_max=1e-5#看lr.py,不用管
C.cos_lr_min=5e-8#看lr.py,不用管
C.cos_lr_total_iters=200000#总共的迭代次数
C.expr_dir=""
C.weight_decay = 1e-4
C.model_pth = None
C.debug=False
