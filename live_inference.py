...
def run_single_inference(latest_16_frames, model, config):
    ...
    future_poses_3d = motion_pred.reshape(b, p, n, config.n_joint, 3) #n = T = number of past frames


def get_skeleton_from_zed():
    # Placeholder: this function must interface with ZED, map skeleton, 
    # and return a (P, 13, 3 ) NumPy array of joint coordinates (or None if no person).
    return None # Must return the latest 3D pose, or None
...
# Keep running getting camera input and inference until we stop
while True:
    # 1. GET DATA (Ensure ZED data processing is defined and working)
    zed_joints = get_skeleton_from_zed()
    
    if zed_joints is not None:

        #Until we have enough history only append
        if len(past_joints) == input_history_length:
            past_joints.pop(0) # Remove the oldest frame
        
        past_joints.append(zed_joints) # Add the new frame (current time is at the end)

        # 3. RUN INFERENCE when buffer is full
        if len(past_joints) == input_history_length:
            future_poses = run_single_inference(past_joints, model, config, device)
            # Use 'future_poses' for whatever downstream task you have
            print(f"Predicted {future_poses.shape[0]} future frames.")
            
    # Add a small sleep or a frame check to control the loop speed
...
#     return future_poses_3d.cpu().numpy().squeeze()
